...
- Working application code with full playfield (tasks/, bugs/, requirements/, roadmap/, docs/)
...
**Behavior:** Read PRD + design decisions. Make architecture choices (framework, database, project structure). Scaffold the full playfield — the project folder structure that every skill operates through. Create Stage 0 tasks from the PRD. This is opinionated — /build picks the stack based on requirements, the business owner doesn't choose React vs Next.js.
...
**Exit:** Playfield scaffolded, docs populated, Stage 0 tasks created, architecture documented, SESH.md updated.
...
### 7.2 The Playfield (BOOTSTRAP creates this)

The **playfield** is the project folder structure that every skill in the playbook operates through. BOOTSTRAP scaffolds the full playfield. Earlier skills created SESH.md + STATUS.md; BOOTSTRAP adds everything else.

```
project-root/                         ← the playfield
  prd/prd.md                    ← from /define (read-only for /build)
  SESH.md                       ← skill-to-skill contract (already exists)
...
**Found:** [YYYY-MM-DD]
**Found by:** [playbook/mode — e.g. /build BUILD, /test HUNT]
...
Bake in sensible CSS transitions by default. No separate animation playbook needed for v1.
...
/build is the longest-running playbook. It will span many sessions. Every session must be self-contained.
...
- [x] Created: [list items completed, e.g., playfield, docs/, tasks/]
...
## What's Done
Your playfield is set up and ready for development.

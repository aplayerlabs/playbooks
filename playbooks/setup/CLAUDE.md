...
Set up the business owner's infrastructure (GitHub, hosting, domain) and create per-project deployment configuration so downstream playbooks can build and ship.
...
- Is read fresh by every playbook at session start
...
When PROJECT mode needs a repo, the playbook runs all git commands on behalf of the business owner. The business owner should never have to understand or type git commands.
...
**Steps (run by the playbook, not the business owner):**
...
> "Done. Your code is now connected to GitHub at [URL]. Every change will be saved there automatically. You don't need to worry about the technical details — the playbooks handle that."
